...
async function runPipeline(runId: string, flags: PipelineFlags): Promise<void> {
  ...
  try {
    ...
    let images: Map<number, string>;
    if (flags.smoke) {
      scenes = scenes.map((s) => ({ ...s, visual_prompt: "synthetic test" }));
      images = await smokeImages(dir, scenes);
      log(runId, "info", "Smoke: synthetic images generated with ffmpeg", "images");
    } else {
      scenes = await describeScenes(runId, dir, scenes);
      checkCancelled(runId);
      images = await generateImages(runId, dir, scenes);
    }
    checkCancelled(runId);

    // 4) RENDER each scene to its window
    log(runId, "info", `Rendering ${scenes.length} clips at ${width}x${height}@${fps}…`, "render");
    const clips = await renderAllScenes(runId, dir, scenes, voice.cues, images, width, height, fps);
    checkCancelled(runId);
    ...
    updateRun(runId, {
      ...
      stats: {
        ...
        uniqueImages: new Set([...images.values()]).size,
