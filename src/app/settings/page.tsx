...
const GROUPS: Group[] = [
  {
    ...
    fields: [
      ...
      {
        key: "PEXELS_API_KEY",
        label: "Pexels API Key (stock video + photos)",
        type: "password",
        hint: "Free at pexels.com/api (~30s). Powers stock footage & photos in the mix. Optional — without it the keyless sources (Openverse/Wikimedia/Archive) still supply stock photos, but Pexels adds real video. Paste several keys (one per line) to raise the rate limit.",
      },
      {
        key: "PIXABAY_API_KEY",
        label: "Pixabay API Key (extra stock source, optional)",
        type: "password",
        hint: "Free at pixabay.com/api/docs. Only used if you add 'pixabay' to Footage sources below.",
      },
    ],
  },
  {
  ...
  {
    title: "Mixed media — stock footage + AI video",
    fields: [
      {
        key: "VISUAL_MIX_ENABLED",
        label: "Mix sources (instead of only AI images)",
        type: "select",
        options: ["true", "false"],
        hint: "On = each scene uses one of four sources by the ratio below (needed to pass a CMS review). Off = the original all-AI-images animated-stills look. Anything that can't be found (no Pexels key, no Veo credits, no match) quietly falls back to an AI image, so a video always assembles.",
      },
      { key: "MIX_AI_IMAGE_PCT", label: "Share — AI images (%)", type: "text" },
      {
        key: "MIX_AI_VIDEO_PCT",
        label: "Share — AI videos (%)",
        type: "text",
        hint: "AI-animated clips (genaipro Veo). These cost Veo credits and are the slowest part — keep this modest. Their scenes are cut short to match the clip length.",
      },
      { key: "MIX_STOCK_VIDEO_PCT", label: "Share — stock videos (%)", type: "text" },
      { key: "MIX_STOCK_IMAGE_PCT", label: "Share — stock photos (%)", type: "text" },
      {
        key: "AI_VIDEO_SCENE_SEC",
        label: "AI-video scene length, sec",
        type: "text",
        hint: "genaipro Veo has no length control — clips come back at a fixed ~8s. AI-video scenes are cut to about this, so the clip fills them with no stretching or freezing.",
      },
      {
        key: "MAX_UNIQUE_AI_VIDEOS",
        label: "Max unique AI video clips per video",
        type: "text",
        hint: "Animated clips are pooled and reused round-robin (like images) so a long video only pays for a dozen, not hundreds.",
      },
      { key: "MAX_UNIQUE_STOCK_IMAGES", label: "Max unique stock photos per video", type: "text" },
      {
        key: "AI_VIDEO_MOTION_PROMPT",
        label: "AI-video motion instruction",
        type: "textarea",
        rows: 3,
        hint: "How genaipro should animate each still — keep it to subtle motion so the mystical image stays recognisable.",
      },
      {
        key: "FOOTAGE_SOURCES",
        label: "Footage sources",
        type: "text",
        hint: "Comma list. Keyless: openverse, wikimedia (photos), archive (video). With a key: pexels (video+photo), pixabay. Default: pexels,openverse,wikimedia,archive",
      },
      {
        key: "REAL_MATCH_THRESHOLD",
        label: "Stock relevance bar (0–100)",
        type: "text",
        hint: "How strict the AI is about a stock clip matching the scene. It cascades (broadens the search if nothing clears the bar). Keep it modest (~55) — too high starves footage and everything falls back to AI.",
      },
      { key: "STOCK_ORIENTATION", label: "Stock orientation", type: "select", options: ["landscape", "portrait", "square"] },
      { key: "STOCK_MAX_HEIGHT", label: "Stock max height, px", type: "text" },
      { key: "STOCK_MIN_DURATION", label: "Stock video min duration, sec", type: "text" },
